...
public:
    void run() {
        vector<TestCase> cases = {
            {{1,2,1,2,3}, 2, 7}, {{1,2,3}, 1, 3}, {{1,2,1,3,4}, 3, 3},
            {{1,1,1,1}, 1, 10}, {{1,2,3,4,5}, 2, 4}, {{1,2,1,2,1,2}, 1, 6},
            {{3,1,4,1,5}, 3, 4}, {{7,7,7,7}, 1, 10}, {{1,2,3}, 3, 1},
            {{1,2,3,4}, 4, 1}, {{2,1,1,1,3}, 2, 6}, {{1,2,3,1,2,3}, 2, 5},
            {{5,5,5,5,5}, 2, 0}, {{1,3,2,3,1}, 2, 5}, {{1,2,1,2,3}, 1, 5}
        };
        Solution sol; int passedCount=0, totalCount=cases.size();
        cout<<Color::BOLD<<"Running "<<totalCount<<" Tests for Exactly K Distinct Integers..."<<Color::RESET<<"\n\n";
        for(int i=0;i<totalCount;i++){
            auto tc=cases[i];
            stringstream buffer; streambuf* old=cout.rdbuf(buffer.rdbuf());
            int result=sol.subarraysWithKDistinct(tc.nums,tc.k);
