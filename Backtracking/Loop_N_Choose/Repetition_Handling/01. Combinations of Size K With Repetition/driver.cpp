...
public:
    void run() {
        vector<TestCase> cases = {
            ...
            { {1,2},   3, {{1,1,1}, {1,1,2}, {1,2,2}, {2,2,2}} },
            { {3},     2, {{3,3}} },
            { {1,5},   2, {{1,1}, {1,5}, {5,5}} },
            { {1,2,3}, 1, {{1}, {2}, {3}} },
            { {1,2},   0, {{}} },
            { {2,4,6}, 2, {{2,2}, {2,4}, {2,6}, {4,4}, {4,6}, {6,6}} },
            { {10},    1, {{10}} },
            { {5,10},  3, {{5,5,5}, {5,5,10}, {5,10,10}, {10,10,10}} },
            { {1,2,3}, 3, {{1,1,1},{1,1,2},{1,1,3},{1,2,2},{1,2,3},{1,3,3},{2,2,2},{2,2,3},{2,3,3},{3,3,3}} },
            { {1},     5, {{1,1,1,1,1}} },
            { {1,2},   1, {{1}, {2}} },
            { {1,2},   2, {{1,1}, {1,2}, {2,2}} },
            { {7,8},   2, {{7,7}, {7,8}, {8,8}} },
            { {1,2,3,4}, 2, {{1,1},{1,2},{1,3},{1,4},{2,2},{2,3},{2,4},{3,3},{3,4},{4,4}} }
        };
        ...
        cout << Color::BOLD << "Running " << totalCount
             << " Tests for Combinations of Size K With Repetition..." << Color::RESET << "\n\n";
        ...
        for (int i = 0; i < totalCount; i++) {
            ...
            vector<vector<int>> result = sol.combineWithRepetition(tc.nums, tc.k);
            ...
            if (tc.k == 0) {
                 bool passed = (result.size() == 1 && result[0].empty());
                 if (passed) {
