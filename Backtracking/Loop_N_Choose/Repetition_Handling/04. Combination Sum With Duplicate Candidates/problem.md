...
- **Input Deduplication**: Since the problem states you can use each element **unlimited times**, having a number appear multiple times in the `candidates` array is redundant.
- To avoid generating duplicate combinations (e.g., using the "first" 2 vs the "second" 2), first **extract the unique elements** from `candidates` and sort them.
- Then, apply the standard **Combination Sum (Unlimited Usage)** backtracking algorithm on the unique elements.
- Pass the current index **`i`** in the recursive call to allow reuse.
