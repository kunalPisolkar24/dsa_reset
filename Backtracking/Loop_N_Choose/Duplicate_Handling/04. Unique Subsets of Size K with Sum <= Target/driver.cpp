...
public:
    void run() {
        vector<TestCase> cases = {
            { {1,2,2,3}, 2, 4, {{1,2}, {1,3}, {2,2}} },
            { {1,2,3,4}, 2, 5, {{1,2}, {1,3}, {1,4}, {2,3}} },
            { {1,1,1},   3, 4, {{1,1,1}} },
            { {5,6,7},   2, 4, {} },
            { {1,1,1},   2, 2, {{1,1}} },
            { {1,2,2},   2, 3, {{1,2}} },
            { {1,2,2},   2, 4, {{1,2}, {2,2}} },
            { {1,1,2,2}, 2, 3, {{1,1}, {1,2}} },
            { {1,1,2,2}, 2, 4, {{1,1}, {1,2}, {2,2}} },
            { {5,5,10,10}, 2, 15, {{5,5}, {5,10}} },
            { {1,2,3,4,5}, 2, 5, {{1,2}, {1,3}, {1,4}, {2,3}} },
            { {10,10,10}, 2, 20, {{10,10}} },
            { {1,1},     1, 1, {{1}} },
            { {2,2,3,3}, 2, 5, {{2,2}, {2,3}, {3,3}} },  // 3,3 sum = 6 > 5, so no. Wait. 2+2=4, 2+3=5, 3+3=6. So only [2,2] and [2,3].
            { {1,1,1,2,2}, 3, 4, {{1,1,1}, {1,1,2}} }
        };
        
        // Correct case 14:
        cases[13] = { {2,2,3,3}, 2, 5, {{2,2}, {2,3}} };
        ...
        cout << Color::BOLD << "Running " << totalCount
             << " Tests for Unique Subsets of Size K with Sum Limit..." << Color::RESET << "\n\n";
        ...
        for (int i = 0; i < totalCount; i++) {
            ...
            vector<vector<int>> result = sol.uniqueSubsetsOfSizeKWithSumLimit(tc.nums, tc.k, tc.target);
