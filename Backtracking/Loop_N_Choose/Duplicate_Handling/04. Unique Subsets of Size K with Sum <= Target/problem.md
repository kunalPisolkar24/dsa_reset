...
- **Pruning**: Since elements are non-negative, if `currentSum + nums[i] > target`, you can `break` the loop early (assuming the array is sorted).
- **Pruning**: If the remaining elements are fewer than needed to reach size `k`, return early.
