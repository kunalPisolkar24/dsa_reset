...
- At each index, **pick** the element (add to current subsequence) or **not-pick** (skip it), then recurse.
- **Pruning**: if the current subsequence already has `k` elements, record it and return.
- **Pruning**: if the remaining elements are not enough to reach `k`, prune that branch early.
- Total output size = `C(n, k)` subsequences.
