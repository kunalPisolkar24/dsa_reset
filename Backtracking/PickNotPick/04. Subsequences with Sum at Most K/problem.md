...
- At each index, **pick** the element (accumulate its value) or **not-pick** (skip it), then recurse.
- **Early pruning**: since all elements are non-negative, if `currentSum > target` at any point, the entire subtree can be pruned.
- Record the current subsequence when: it is non-empty **and** its sum is `<= target`.
- Total output size varies based on input; worst case is `2^n - 1` (all non-empty subsequences).
